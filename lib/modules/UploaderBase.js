...
class UploaderBase {
  ...
  _initializeInputRecordComplete(response) {
    console.log('initialize input record is done!')
    const data = JSON.parse(response.data)

    this.initialized = true

    this.uploadId = data.uploadId
    console.log('this from initialize input rec comp', data)
    this.key = data.key
  ...
  _signChunk(chunk) {
    ...
    // Test pausing during sign chunk
  ...
  _sendChunk(chunk, response) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Test Pausing during send chunk
      // if(this.pauseCount === 0) {
      //   this.pauseCount++
      //   this.pause()
      //   setTimeout(() => this.resume(), 1000)
