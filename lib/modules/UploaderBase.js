...
class UploaderBase {
   ...
   */
  abort () {
    throw new Error('abort unimplemented')
  }
  ...
  save (callback) {
    ...
    const promise = this._createInputRecord()
  ...
  _createInputRecord () {
    ...
    let promise = this.api.Inputs.add({
      filename: this.file.name,
      type: this.file.type,
      size: this.file.size
    })

    return promise.then(this._createInputRecordSuccess.bind(this))
  }
   ...
   * @private
   * @param   {object}  chunk    - the upload with data attached
