...
class Uploader {
  ...
  constructor (options) {
    ...
    this.singlePartCancellable = null
  ...
  pause () {
    console.log('pause')
    console.log('Is upload complete?', this.uploadComplete)
    ...
    // console.log(this.singlePartCancellable)

    // Abort the upload if its a singlepart upload
    if (this.singlePartCancellable) {
      console.log('Single part cancellable exists, cancelling');
      this.singlePartCancellable.abort()
    } else {
  ...
  resume () {
    ...
    if (this.serialPromise) {
      ...
      console.log('resuming multipart')
    } else if (this.singlePartPromise) {
      console.log('rsuming singlepart')
  ...
  //     console.log('c=single part abort')
  //     this.singlePartPromise = null
  ...
  _uploadSinglepartFile () {
    ...
    this.singlePartPromise = new Promise((_resolve, _reject) => {
      this.singlePartResolve = _resolve
      this.singlePartReject = _reject
    })
  ...
  _sendSinglePart() {
    // This will allow us to cancel single part uploads without breaking the initial chain.
    this._signChunk(this.file)
      .then((response) => {
        return this._sendChunk(this.file, response)
        console.log('from send chunk', this.singlePartPromise)
        console.log('upload complete from send chunk', this.uploadComplete)

        // setTimeout(() => {
        //   this.pause()
        // }, 500)
        // setTimeout(this.resume.bind(this), 1000)
      })
  ...
  _sendChunk (chunk, response) {
    ...
    return new Promise((resolve, reject) => {
      this.singlePartCancellable = request(options, (error, response, body) => {
        if (error) {
          console.log(error);
          return reject(error)
        } else {
      ...
      return this.singlePartCancellable
    })
  }
