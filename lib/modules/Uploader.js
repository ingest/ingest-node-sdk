...
class Uploader {
  ...
  _initializeInputRecord () {
    ...
    url = Utils.parseTokens(this.config.upload, tokens);
  ...
  // Step 7, Do all the things for each chunk
  _createChunks  () {
  ...
  _signChunk (chunk) {
    ...
    url = Utils.parseTokens(this.config.sign, {
      id: this.fileRecord.id,
      method: signing
    });
  ...
  _sendChunk (upload, response) {
    ...
    // Parse the sign chunk response, this is step 4
  ...
  /**
   * Checks is the file is a buffer (a normal file) and converts it to a stream if it is.
   */
