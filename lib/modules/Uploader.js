...
class Uploader {
  ...
  pause () {
    ...
    console.log(this.singlePartPromise)

    // Abort the upload if its a singlepart upload
    if (this.singlePartPromise) {
      console.log('Single part promise')
      this.singlePartPromise.cancel()
    }
  }
  ...
  _uploadSinglepartFile () {
    ...
    stream.on('end', () => {
      ...
      this._signChunk(this.file)
        .then((response) => {
          this._sendChunk(this.file, response)
          console.log('from send chunk', this.singlePartPromise)
          console.log('uplaod complete from send chunk', this.uploadComplete)
