...
class Request {
  ...
  makeRequest () {
    ...
    result.request = new Promise((resolve, reject) => {
      activeRequest = request(options, this.requestComplete.bind(this, reject, resolve))
    })
  ...
  requestComplete (reject, resolve, error, response, body) {
    ...
    if (!validResponse) {
      console.log('failed request', response.statusCode)
      return reject({
