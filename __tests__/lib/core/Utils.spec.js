...
describe('Utils Tests', () => {
  ...
  const payload_error_token = 'Bearer test.test'
  const null_token = 'Bearer test'
  ...
  describe('Utils::parseTokenPayload', () => {
    ...
    it('Should return null if the token is invalid', () => {
      const result = Utils.parseTokenPayload(null_token)
      expect(result).toBeNull()
    })

    it('Should return null if parsed payload errors', () => {
      const result = Utils.parseTokenPayload(payload_error_token)
      expect(result).toBeNull()
    })
  })
